...
thanh_vien <- dt_join %>% 
  ...
  mutate(gender = case_when(m1ac2 == 1 ~ 'Nam', TRUE ~ 'Nữ'),
         ...
         quanhe = case_when(m1ac3 == 1 ~ 'Chủ hộ',
                            ...
                            TRUE ~ 'Khác'),
         hhincome = thunhap,
         yearly_wageA = m4ac11 + m4ac12f, # Tiền lương từ công việc chính
         yearly_wage = m4ac11 + m4ac12f + m4ac21 + m4ac22f + m4ac25, # Tiền lương tất cả các CV
         hhwage = m4atn,
         # Trình độ học vấn cao nhất
         highest_degree = case_when(m2ac3a == 0 ~ 'Không bằng cấp',
                                    m2ac3a == 1 ~ 'Tiểu học',
                                    m2ac3a == 2 ~ 'THCS',
                                    m2ac3a == 3 ~ 'THPT',
                                    m2ac3a == 8 ~ 'Cao đẳng',
                                    ...
                                    m2ac3a == 11 ~ 'Tiến sỹ',
                                    TRUE ~ 'Khác'),
         train =  case_when(m2ac3b == 0 ~ 'Không',
         ...
         degree_now = case_when(m2ac8 == 0 ~ 'Nhà trẻ, mẫu giáo',
                                m2ac8 == 1 ~ 'Tiểu học',
                                m2ac8 == 2 ~ 'THCS',
                                m2ac8 == 3 ~ 'THPT',
                                m2ac8 == 4 ~ 'Sơ cấp nghề',
         ...
         economic_sector = case_when(m4ac10a == 2 ~ 'Hộ NLT/ Cá nhân',
                                     m4ac10a == 1 | m4ac10a == 3 ~ 'Hộ SXKD cá thể',
                                     m4ac10a == 5 ~ 'Tập thể',
                                     m4ac10a == 6 ~ 'Tư nhân',
                                     m4ac10a == 4 ~ 'Nhà nước',
         ...
         quint = qcut(thubq),
         # quint_ct = cut(chibq),
         dung_tieuhoc = case_when((edu >=1 & edu <=5) & (age >=7 & age <=11) ~ 100,
                                  TRUE ~ 0),
         dung_thcs = case_when((edu >=6 & edu <=9) & (age >=12 & age <=15) ~ 100,
                               TRUE ~ 0),
         dung_thpt = case_when((edu >=10 & edu <=12) & (age >=16 & age <=18) ~ 100,
                               TRUE ~ 0),
         nuocsach = case_when(m7c26 %in% c(6, 8, 13, 14) ~ 0, TRUE ~ 100), 
         loainha = case_when(m7c3 == 1 ~ 'Nhà biệt thự',
