...
# Import all file
file_vhlss_2010 <- Sys.glob("vhlss2010/*.dta")
dt <- rio::import_list(file_vhlss_2010)
...
dt_join <- dt$muc1a %>%
  ...
  inner_join(dt$muc2_qx_cau9_11) %>%
  inner_join(dt$muc4a1) %>%
  inner_join(dt$muc4a2) %>%
  inner_join(dt$muc4a3) %>%
  inner_join(dt$muc4a4) %>%
  inner_join(dt$muc4a5) %>%
  inner_join(ho) %>%
  inner_join(dt$weight10)
...
thanh_vien <- dt_join %>%
  ...
  mutate(
    ...
    quint = qcut(thubq),
    # quint_ct = cut(chibq),
    dung_tieuhoc = case_when((edu_now >= 1 &
                                edu_now <= 5) & (age >= 6 & age <= 10) ~ 100,
                             TRUE ~ 0),
    dung_thcs = case_when((edu_now >= 6 &
                             edu_now <= 9) & (age >= 11 & age <= 14) ~ 100,
                          TRUE ~ 0),
    dung_thpt = case_when((edu_now >= 10 &
                             edu_now <= 12) & (age >= 15 & age <= 17) ~ 100,
                          TRUE ~ 0),
    nuocsach = case_when(m7c14  == 5 |
                           m7c14  == 7 | m7c14  == 10 ~ 0, TRUE ~ 100),
    cot = case_when(m7c3 >= 1 & m7c3 <= 3 ~ 1, TRUE ~ 0),
    mai = case_when(m7c4 >= 1 & m7c4 <= 2 ~ 1, TRUE ~ 0),
    tuong = case_when(m7c5 >= 1 & m7c5 <= 3 ~ 1, TRUE ~ 0),
    loainha = case_when(
      cot + mai + tuong == 3 ~ 'Nhà kiên cố',
      cot + mai + tuong == 2 ~ 'Nhà bán kiên cố',
      cot + mai + tuong == 1 ~ 'Nhà thiếu kiên cố',
      cot + mai + tuong == 0 ~ 'Nhà đơn sơ',
      TRUE ~ 'Missing'
    ),
